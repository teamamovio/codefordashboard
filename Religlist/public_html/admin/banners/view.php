...
if (!( isset($_GET['id']) && is_string($_GET['id']) && preg_match('#^\\d{1,20}$#Duis', $_GET['id']) )) {
	header("Location: {$www_base}/admin/banners/");
	exit;
}
$bannerId = $_GET['id'];
...
$bannerChurches = array();
$q = mysql_query(
	"SELECT church_id FROM banners_churches ".
	"WHERE banner_id = '".mysql_real_escape_string($bannerId, $db)."'",
	$db
) or err("MySQL error: ".mysql_error($db), __FILE__, __LINE__);
while ($row = mysql_fetch_assoc($q)) $bannerChurches[] = $row['church_id'];
mysql_free_result($q);
...
$f = array();
$f['html'] = (isset($_POST['html']) && is_string($_POST['html'])) ? mb_trim($_POST['html']) : $bannerRow['html'];
...
<h1 align="center">Update Banner</h1>
